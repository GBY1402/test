...
<div class="pageContent">
	 <form method="post" action="${ctx }/resume/skillpoint" class="pageForm required-validate" onsubmit="return validateCallback(this, dialogReloadNavTab)">
		<div class="pageFormContent" layoutH="56">
				<label>技能点选择：</label>
				<input type="hidden" name="resumeId" value="${sid }">
           <c:forEach items="${map }" var="skillsInfoslist" varStatus="fskill">
			<p>
			<label>
           <c:forEach items="${skillsInfoslist.value }" var="skillsInfos" varStatus="st">
			 <c:if test="${st.index==1}">
			 ${skillsInfos.industrySkillsInfo.name }
			 </c:if>
			</c:forEach>
           </label>
           <input type="hidden" name="id" value="${sid }">
			<c:forEach items="${skillsInfoslist.value }" var="skillsInfos"  >
			 <input type="radio"   type="hidden" name="SkillsInfos[${fskill.index }].sId" value="${skillsInfos.sId }"/>${skillsInfos.sName }
			</c:forEach>
			</p>
			</c:forEach>
		</div>
