...
<form id="pagerForm" method="post" action="${ctx }/recruit/hrLookupByWallet">
...
<form id="searchForm" method="post" action="${ctx}/recruit/hrLookupByWallet" onsubmit="return dialogSearch(this)">
...
<table class="table" layoutH="137">
    ...
    <tbody>
        <c:forEach items="${page.content}" var="hr">
            <tr target="sid_user" rel="${hr.enterPersonInfo.enterPerId}">
                 <td>${hr.enterPersonInfo.humanBasicInfo.humanName }</td>
                <td>${hr.enterPersonInfo.enterPosition }</td>
                <td>${hr.enterPersonInfo.personAndbasicInfo.enterBasicInfo.enterName }</td>
                <td>
                     <a class="btnSelect" href="javascript:$.bringBack({'walletID':'${hr.walletID }','enterPersonInfo.humanBasicInfo.humanName':'${hr.enterPersonInfo.humanBasicInfo.humanName }'})" title="用户信息">选择</a>
                 </td>
            </tr>
        </c:forEach>
    </tbody>
