...
<div class="pageContent">
    <form method="post" action="${ctx}/recruit/worry/update" enctype="multipart/form-data" class="pageForm required-validate" onsubmit="return iframeCallback(this,dialogReloadNavTab)">
        ...
        <div class="pageFormContent" layoutH="50">
           ...
           <div class="unit">
                <label>人数：</label>
				<input name="numberJob" class="validate[required,maxSize[32]] required" size="30" value="${enter.numberJob }" />
           </div>
           <div class="unit">
                <label>佣金：</label>
				<input name="commission" class="validate[required,maxSize[32]] required" size="30" value="${enter.commission }" />
           </div>
           <div class="unit">
                <label>到岗开始时间：</label>
				<input name="workArrivalStart" type="text" class="validate[required,custom[date],past[#regulartime]] text-input date required" size="30" value='<fmt:formatDate value="${enter.workArrivalStart}" pattern="yyyy-MM-dd"/>' readonly="readonly" minDate="{%y-100}-%M-%d" maxDate="{%y+100}-%M-{%d}" /><a class="inputDateButton" href="javascript:;">选择</a>
           </div>
           <div class="unit">
                <label>到岗结束时间：</label>
				<input name="workArrivalEnd" type="text" class="validate[required,custom[date],past[#regulartime]] text-input date required" size="30" value='<fmt:formatDate value="${enter.workArrivalEnd}" pattern="yyyy-MM-dd"/>' readonly="readonly" minDate="{%y-100}-%M-%d" maxDate="{%y+100}-%M-{%d}" /><a class="inputDateButton" href="javascript:;">选择</a>
           </div>
           <div class="unit">
                <label>详细地址：</label>
           ...
           <div class="unit">
			...
			<div class="unit">
                ...
                <option value="" selected>请选择</option>
                <option  ${enter.jobDelete == '0' ? 'selected' : '' } value="0">未删除</option>
                <option  ${enter.jobDelete == '1' ? 'selected' : '' } value="1">已删除</option>
                </select>
                <span><font color="red">*</font></span>
           </div>
           <div class="unit">
                ...
                <a class="btnLook" rel="addskill" width="500" height="500" href="${ctx }/recruit/hrLookup?enterDelete=0" lookupGroup="enterPersonInfo" title="选择用户" >查找带回</a>
            </div>
        </div> 
        <div class="formBar">
            <ul>
