...
<form id="pagerForm" method="post" action="${ctx }/recruit/worry">
	<input type="hidden" name="numPerPage" value="${page.size}" />
	<input type="hidden" name="pageNum" value="${page.number+1 }" />
</form>

<form id="searchForm" method="post" action="${ctx}/recruit/worry" onsubmit="return navTabSearch(this)">
	<input type="hidden" name="search_EQ_jobAttribute" value="1" />
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
		<shiro:hasPermission name="RecruitJ:save">
			<li><a  width="550" height="600" rel="companyback" class="add" href="${ctx}/recruit/worry/save" target="dialog" mask="true"><span>添加</span></a></li>
		</shiro:hasPermission>
		<shiro:hasPermission name="RecruitJ:edit">
			<li><a  width="550" height="600" rel="resume_edit" class="edit" href="${ctx}/recruit/worry/${emp.jobWanfedId}/{sid_user}/update" target="dialog" ><span>修改</span></a></li>
		</shiro:hasPermission>
		<li class="line">line</li>
		<shiro:hasPermission name="RecruitJ:view">
			<li><a class="icon" width="850" height="450" href="${ctx}/recruit/worry/${emp.jobWanfedId}/{sid_user}/view" target="dialog" mask="true"><span>详细</span></a></li>
		</shiro:hasPermission>
		<shiro:hasPermission name="RecruitJ:delete">
			<li><a class="delete" href="${ctx}/recruit/worry/${emp.jobWanfedId}/{sid_user}/delete" target="ajaxTodo" title="确定要删除吗?"><span>删除</span></a></li>
		</shiro:hasPermission>
		</ul>
	</div>
...
<table class="table" layoutH="137">
    <thead>
        <tr >
            ...
            <th width="150px" align="center">到岗开始时间</th>
            <th width="150px" align="center">到岗结束时间</th>
    ...
    <tbody>
        <c:forEach items="${page.content}" var="emp">
            <tr target="sid_user" rel="${emp.jobWanfedId}">
                  ...
                  <td><fmt:formatDate value="${emp.workArrivalStart}" type="date" dateStyle="long"/></td>
                  <td><fmt:formatDate value="${emp.workArrivalEnd}" type="date" dateStyle="long"/></td>
