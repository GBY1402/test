...
<form id="pagerForm" method="post" action="${ctx }/recruit">
	<input type="hidden" name="numPerPage" value="${page.size}" />
	<input type="hidden" name="pageNum" value="${page.number+1 }" />
</form>

<form id="searchForm" method="post" action="${ctx}/recruit" onsubmit="return navTabSearch(this)">
	<input type="hidden" name="search_EQ_jobAttribute" value="0" />
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
		<shiro:hasPermission name="Recruit:save">
			<li><a  width="550" height="500" rel="companyback" class="add" href="${ctx}/recruit/save" target="dialog" mask="true"><span>添加</span></a></li>
		</shiro:hasPermission>
		<shiro:hasPermission name="Recruit:edit">
			<li><a  width="550" height="500" rel="resume_edit" class="edit" href="${ctx}/recruit/${emp.jobWanfedId}/{sid_user}/update" target="dialog" ><span>修改</span></a></li>
		</shiro:hasPermission>
		<li class="line">line</li>
		<shiro:hasPermission name="Recruit:view">
			<li><a class="icon" width="850" height="400" href="${ctx}/recruit/${emp.jobWanfedId}/{sid_user}/view" target="dialog" mask="true"><span>详细</span></a></li>
		</shiro:hasPermission>
		<shiro:hasPermission name="Recruit:delete">
		<li><a class="delete" href="${ctx}/recruit/${emp.jobWanfedId}/{sid_user}/delete" target="ajaxTodo" title="确定要删除吗?"><span>删除</span></a></li>
		</shiro:hasPermission>
		</ul>
	</div>
